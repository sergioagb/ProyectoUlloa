...
<v-card class="pa-5">
<v-card-title>Añadir este Celular </v-card-title>
<v-divider></v-divider>
<v-form ref="form" @submit.prevent="updateForm" class="pa-5" encrype="multipart/form-data">
...
<v-file-input @change="selectFile"  show-size counter multiple label="SelectImage" ></v-file-input>
...
export default{
    data(){
        return{
            rules: [(value)=>!!value // "This field is requiered!"],
post:{
    title:"",
    category:"",
    content:"",
...
image:"",
    ...
    async created(){
con response = await API.getPostByID(this.$route.params.id);
this.post = response;
    },
methods:{
    selectFile(file){
